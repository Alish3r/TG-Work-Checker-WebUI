...
# Reuse config + parsing, but we will force OUTPUT_DB via CLI
from scrape_telegram import init_db, load_config, _topic_id_norm


async def backfill(config):
    ...
    print("One-time backfill into a separate DB...")
...
def main():
    parser = argparse.ArgumentParser(
        description="One-time backfill: scrape full Telegram history into its own separate SQLite DB."
    )
    parser.add_argument("--chat", required=True, help="Chat identifier: username/@username or t.me link.")
    parser.add_argument("--output-db", required=True, help="Output SQLite DB path for this single topic.")
